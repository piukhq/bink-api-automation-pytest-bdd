@asos
Feature: Merchant asos - Ensure a customer can add their payment card & link it to loyalty membership card
  ...
  Scenario Outline: ADD & LINK Journey_Asos
    ...
    Then I perform GET request to verify the "Asos" membership card is added & linked successfully in the wallet
  ...
  Scenario: PLL Link by PATCH_(mcard_pcard)_Asos
    ...
    And I perform the GET request to verify the payment card has been added successfully to the wallet
    When I perform POST request to add "Asos" membership card to my wallet
    ...
    And I perform GET request to verify the "Asos" membership card is linked successfully in the wallet
    ...
    And I perform GET/membership_card/id request to verify the payment card is unlinked from "Asos" membership card
...
#    Below visa paymentcard scenario run when staging is pointing to pelops only
  ...
  Scenario Outline: Enrol link and unlink the visa vop in pelops for Asos
    ...
    When I perform POST request to add & auto link "Asos" membership card
    Then I perform GET request to verify the "Asos" membership card is added & linked successfully in the wallet
    Then I verify status of paymentcard is "activated" for "Asos"
    And I perform DELETE request to delete the payment card
    And I verify status of paymentcard is "deactivated" for "Asos"
