@viator @sanity
Feature: Merchant Viator - Ensure a customer can add their payment card & link it to loyalty membership card
  and view their card(s) details

  As a customer
  I want to utilise payment_cards endpoint
  So I can add my payment card and can link to Viator membership card & check the details successfully

  @bink_regression
  Scenario Outline: ADD & LINK Journey_Viator
    ...
    Then I perform GET request to verify the "Viator" membership card is added & linked successfully in the wallet
  ...
  Scenario: PLL Link by PATCH_(mcard_pcard)_Viator
    ...
    And I perform the GET request to verify the payment card has been added successfully to the wallet
    When I perform POST request to add "Viator" membership card to my wallet
    ...
    And I perform GET request to verify the "Viator" membership card is linked successfully in the wallet
    ...
    And I perform GET/membership_card/id request to verify the payment card is unlinked from "Viator" membership card
    And I perform GET/payment_card/id request to verify the membership card is unlinked

  @patch_pcard_mcard @bink_regression
  Scenario: PLL Link by PATCH_(pcard_mcard)_Viator
  ...
  Scenario Outline: Enrol new paymentcard and link to Viator
  ...
  @enrol_new_paymentcard @bink_regression
  Scenario Outline: Enrol link and unlink the visa vop in pelops for Viator
    ...
    When I perform POST request to add & auto link "Viator" membership card
    Then I perform GET request to verify the "Viator" membership card is added & linked successfully in the wallet
    Then I verify status of paymentcard is "activated" for "Viator"
    And I perform DELETE request to delete the payment card
    And I verify status of paymentcard is "deactivated" for "Viator"
