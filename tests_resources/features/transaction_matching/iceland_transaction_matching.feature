...
Feature: Merchant Iceland - Ensure a customer can use Bink's Transaction Matching features
  ...
  Scenario Outline: Verify transaction matching for iceland with payment provider
    ...
    When I send merchant Tlog file with "<merchant_container>" "<payment_card_provider>" "<mid>" "<card_identity>" and send to bink
  ...
  Scenario Outline: Verify transaction matching for iceland with payment provider but invalid mid
    ...
    Examples:
      ...
      | amex                  | scheme/iceland/    | 7821802109 | Amex                      | amex-settlement-matching   | AMEX       |
      | amex                  | scheme/iceland/    | 8445909831 | Amex                      | amex-auth-matching         | AMEX       |
      | visa                  | scheme/iceland/    | 80209723   | Visa                      | visa-auth-matching         | VISA       |
