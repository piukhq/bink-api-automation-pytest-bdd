...
# import tests.helper.test_data_utils as test_data
...
def add_membership_card(merchant, login_user, context):
    ...
    try:
        ...
        # logging.error('Add Journey for ', merchant, ' failed due to error '+str(error))
...
@when(parsers.parse(
    'I perform GET request to verify "{merchant}" membership card is added & linked successfully in the wallet'))
