...
def ubiquity_verify_payment_card_added(channel_name):
    ...
    if channel_name == "channel_2":
        assert response_json["membership_cards"][0]["id"] == TestContext.scheme_account_id1 and response_json[
            "membership_cards"
        ][0]["active_link"] == PaymentCardTestData.get_data().get(
...
def ubiquity_verify_membership_card_1_is_add_and_linked(merchant, channel):
    response = MembershipCards.get_scheme_account_auto_link(TestContext.token, TestContext.scheme_account_id1)
    response_json = response.json()
    logging.info(
        "The response of GET/MembershipCard after Membership card Add & AutoLink is:\n\n"
        + Endpoint.BASE_URL
        + api.ENDPOINT_MEMBERSHIP_CARD.format(TestContext.scheme_account_id1)
        + "\n\n"
        + json.dumps(response_json, indent=4)
    )
...
@then(parsers.parse('There is no link created between payment_card_1 and "{merchant}" membership_card_2'))
...
def ubiquity_verify_link(merchant):
    response_membership_card_1 = MembershipCards.get_scheme_account(TestContext.token, TestContext.scheme_account_id1)
