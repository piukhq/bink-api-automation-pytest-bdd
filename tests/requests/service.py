...
from tests.payload.service.customer_accounts import CustomerAccounts
...
class CustomerAccount:
    ...
    def create_user(test_email, channel):
        ...
        if channel == config.BINK.channel_name:
            payload = CustomerAccounts.register_user_payload(test_email, config.BINK.client_id,
                                                    config.BINK.bundle_id)
        elif channel == config.BARCLAYS.channel_name:
            payload = CustomerAccounts.register_user_payload(test_email, config.BARCLAYS.client_id,
    ...
    def create_consent(token, test_email):
        ...
        payload = CustomerAccounts.consent_user_payload(test_email)
        return Endpoint.call(url, headers, "POST", payload)
    ...
    def login_user(channel):
        ...
        headers = Endpoint.request_header()
        if channel == config.BINK.channel_name:
            payload = CustomerAccounts.login_user_payload(config.BINK.client_id,
                                                 config.BINK.bundle_id)
        elif channel == config.BARCLAYS.channel_name:
            payload = CustomerAccounts.login_user_payload(config.BARCLAYS.client_id,
                                                 config.BARCLAYS.bundle_id)
        return Endpoint.call(url, headers, "POST", payload)
